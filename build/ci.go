...
import (
	...
	"github.com/dubaicoin-dbix/go-dubaicoin/internal/build"
)

var (
	// Files that end up in the gdbix*.zip archive.
	gethArchiveFiles = []string{
		"COPYING",
		executablePath("gdbix"),
	}

	// Files that end up in the gdbix-alltools*.zip archive.
	...
	debExecutables = []debExecutable{
		{
			Name:        "gdbix",
			Description: "Dubaicoin CLI client.",
		},
		{
			Name:        "rlpdump",
		...
		{
			Name:        "swarm",
			Description: "Duabicoin Swarm daemon and tools",
		},
		{
			Name:        "abigen",
...
func doArchive(cmdline []string) {
	...
	if err := build.WriteArchive(gdbix, gethArchiveFiles); err != nil {
		log.Fatal(err)
	}
	if err := build.WriteArchive(alltools, allToolsArchiveFiles); err != nil {
		log.Fatal(err)
	}
	for _, archive := range []string{gdbix, alltools} {
...
func makeWorkdir(wdflag string) string {
	...
	if wdflag != "" {
		...
		wdflag, err = ioutil.TempDir("", "gdbix-build-")
	}
	if err != nil {
		log.Fatal(err)
	}
...
func newDebMetadata(distro, author string, env build.Environment, t time.Time) debMetadata {
	if author == "" {
		...
		author = "Dubaicoin Builds <rkanamail@gmail.com>"
	}
	return debMetadata{
		Env:         env,
		Author:      author,
...
func doWindowsInstaller(cmdline []string) {
	...
	*workdir = makeWorkdir(*workdir)
	...
	for _, file := range allToolsArchiveFiles {
		...
		if filepath.Base(file) == "gdbix.exe" {
			gethTool = file
		} else {
	...
	installer, _ := filepath.Abs("gdbix-" + archiveBasename(*arch, env) + ".exe")
	build.MustRunCommand("makensis.exe",
		...
		"/DARCH="+*arch,
		filepath.Join(*workdir, "gdbix.nsi"),
	)
...
func doAndroidArchive(cmdline []string) {
	...
	build.MustRun(gomobileTool("bind", "--target", "android", "--javapkg", "org.dubaicoin", "-v", "github.com/dubaicoin-dbix/go-dubaicoin/mobile"))

	if *local {
		// If we're building locally, copy bundle to build dir and skip Maven
		os.Rename("gdbix.aar", filepath.Join(GOBIN, "gdbix.aar"))
		return
	}
	...
	archive := "gdbix-" + archiveBasename("android", env) + ".aar"
	os.Rename("gdbix.aar", archive)
...
func doXCodeFramework(cmdline []string) {
	...
	bind := gomobileTool("bind", "--target", "ios", "--tags", "ios", "-v", "github.com/dubaicoin-dbix/go-dubaicoin/mobile")
	...
	archive := "gdbix-" + archiveBasename("ios", env)
	...
	if *deploy != "" {
		...
		build.Render("build/pod.podspec", "Gdbix.podspec", 0755, meta)
		build.MustRunCommand("pod", *deploy, "push", "Gdbix.podspec", "--allow-warnings", "--verbose")
	}
}
